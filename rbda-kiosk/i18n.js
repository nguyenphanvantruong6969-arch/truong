...
(function (global) {
  ...
  const UI_STRINGS = {
    vi: {
      ...
      confirm_overwrite_mot_phan: "Sẽ xếp lại {buoi} và GHI ĐÈ kết quả cũ của những buổi đó. Các buổi còn lại giữ nguyên. Xác nhận?",
      ...
      chon_buoi_title: "Buổi sẽ xếp",
      chon_buoi_tu: "Từ",
      chon_buoi_den: "đến",
      chon_buoi_tat_ca: "Chọn tất cả",
      chon_buoi_nhac: "Buổi không chọn giữ nguyên kết quả của lần xếp trước.",
      chon_buoi_nhac_giu: "Giữ nguyên kết quả đã có của: {buoi}. Chỉ những buổi đang chọn mới được xếp lại.",
      chon_buoi_nhac_het: "Đang xếp lại toàn bộ các buổi trong tuần.",
      tham_title: "Số bốc thăm theo buổi",
      tham_desc: "Mỗi buổi có một thứ tự riêng, xáo ra từ chính bộ số đã khoá. Nhờ vậy em rút phải số xấu không bị đứng cuối ở mọi buổi.",
      tham_tai_lap: "Trường chỉ cần công bố bộ số đã khoá và con số gieo là đủ: thứ tự từng buổi suy ra từ hai thứ đó, ai tính lại cũng phải ra đúng bảng này.",
      tham_cach_cu: "Lần chạy gần nhất dùng cách bốc thăm cũ — một bộ số dùng chung cho cả tuần — nên các cột dưới đây giống hệt nhau. Chạy lại phân bổ để chuyển sang cách xáo theo từng buổi.",
    ...
    en: {
      ...
      confirm_overwrite_mot_phan: "This will allocate {buoi} again and OVERWRITE the existing results for those sessions. Every other session is left as it is. Confirm?",
      ...
      chon_buoi_title: "Sessions to allocate",
      chon_buoi_tu: "From",
      chon_buoi_den: "to",
      chon_buoi_tat_ca: "Select all",
      chon_buoi_nhac: "Sessions you leave out keep the results from the previous allocation.",
      chon_buoi_nhac_giu: "Keeping the existing results for {buoi}. Only the selected sessions are allocated again.",
      chon_buoi_nhac_het: "Every session in the week will be allocated again.",
      tham_title: "Lottery order by session",
      tham_desc: "Every session has its own order, reshuffled from the same locked draw. That is what stops an unlucky number from putting a student at the back all week.",
      tham_tai_lap: "Publishing the locked draw and the seed is enough: each session's order follows from those two, so anyone who recomputes it must land on this exact table.",
      tham_cach_cu: "The latest run used the earlier design — one draw shared by the whole week — which is why the columns below are identical. Run the allocation again to move to a per-session shuffle.",
