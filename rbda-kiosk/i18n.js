...
(function (global) {
  ...
  const UI_STRINGS = {
    vi: {
      ...
      import_hint: "Kéo thả file Excel (.xlsx) hoặc CSV vào ô dưới — phần mềm tự biết đó là danh sách CLB, file chọn CLB muốn thi hay file xếp hạng nguyện vọng. Thả được nhiều file một lúc, và tự nhập theo đúng thứ tự.",
      drop_zone_main: "Kéo file Excel hoặc CSV vào đây",
      drop_zone_sub: "nhận .xlsx và .csv — bấm để chọn, có thể chọn nhiều file",
      ...
      danger_zone_desc: "Nạp file chỉ CỘNG THÊM học sinh, không xoá em cũ. Muốn chạy thử lại từ đầu với bộ dữ liệu khác thì xoá ở đây, nếu không học sinh của lần trước vẫn chiếm suất và làm lệch kết quả.",
