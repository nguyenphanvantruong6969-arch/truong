...
(function (global) {
  ...
  const ERROR_MESSAGES = {
    ...
    error_running_pipeline: {
      ...
      en: "Unexpected error while running the allocation: {detail}",
    },
    ...
    xlsx_support_missing: {
      vi: "Bản cài này không đọc được tệp Excel. Hãy lưu tệp sang định dạng CSV UTF-8 rồi nạp lại.",
      en: "This build cannot read Excel files. Save the file as CSV UTF-8 and try again.",
    },
    ...
    csv_score_without_club: {
      vi: "Học sinh {student_id} có điểm ở ô điểm số {cot} nhưng ô mã CLB tương ứng lại để trống — nhiều khả năng gõ lệch cột. Điểm đó bị bỏ qua.",
      en: "Student {student_id} has a score in score slot {cot} but the matching club column is empty — the columns are probably misaligned. That score is skipped.",
    },
    ...
    reset_confirmation_mismatch: {
      vi: "Chưa xoá gì cả — thiếu xác nhận. Muốn xoá thật thì phải gõ đúng chữ \"{can_go}\".",
      en: "Nothing was deleted — confirmation missing. To confirm a real reset, type exactly \"{can_go}\".",
    },
    reset_scope_unknown: {
      vi: "Chưa xoá gì cả — yêu cầu xoá không hợp lệ. Hãy dùng đúng nút xoá trong ứng dụng thay vì gọi trực tiếp.",
      en: "Nothing was deleted — the reset request was invalid. Use the reset buttons in the app instead of calling this directly.",
    },
    ...
    csv_missing_columns: {
      vi: "CSV dạng \"dài\" cần có cột mã học sinh (student_id) và mã CLB (club_id) — cột hiện có trong tệp: {fieldnames}",
      en: "\"Long\" format CSV needs a student ID column (student_id) and a club ID column (club_id) — columns found in the file: {fieldnames}",
    },
    capacity_must_be_positive: {
      vi: "Tổng chỗ phải lớn hơn 0",
      en: "Total seats must be greater than 0",
    },
    no_data_to_run: {
      vi: "Chưa có dữ liệu để chạy thử. Cần ít nhất một câu lạc bộ và một học sinh.",
      en: "No data to try yet. At least one club and one student are required.",
    },
    clb_thieu_buoi: {
    ...
    cannot_delete_club_referenced: {
      ...
      en: "Cannot delete: {n_prefs} student preference(s) and {n_matches} match result(s) still point to club {club_id}. Handle that data first.",
    },
    student_not_found: {
    ...
    scores_must_be_nonempty_list: {
      vi: "Danh sách điểm không được để trống",
      en: "The list of scores cannot be empty",
    },
    student_ids_must_be_nonempty_list: {
      vi: "Danh sách mã học sinh không được để trống",
      en: "The list of student IDs cannot be empty",
    },
    club_ids_must_be_list: {
      vi: "Danh sách mã CLB không hợp lệ",
      en: "The list of club IDs is invalid",
    },
    ordered_club_ids_must_be_list: {
      vi: "Danh sách xếp hạng CLB không hợp lệ",
      en: "The ranked club list is invalid",
    },
    ...
    cannot_delete_student_matched: {
      ...
      en: "Cannot delete: student {student_id} is already in the results of the latest allocation run. Re-run the allocation after handling this.",
    },
    ...
    rbda_done: {
      vi: "Hoàn tất, không phát sinh lỗi ({rounds} vòng xử lý).",
      en: "Completed with no errors ({rounds} processing round(s)).",
    },
    db_backed_up: {
    ...
    pipeline_rolled_back: {
      ...
      en: "Undid every change from this run (including any freshly drawn STB numbers) because of a mid-run error — data is back to exactly the state before you clicked run.",
    },
    pref_student_not_in_students: {
      vi: "Học sinh {student_id} có nguyện vọng nhưng không có trong danh sách học sinh",
      en: "Student {student_id} has preferences but is not in the student list",
    },
    pref_duplicate_club: {
    ...
    club_capacity_not_positive: {
      vi: "CLB {club_id} có tổng chỗ bằng 0 hoặc âm — phải lớn hơn 0",
      en: "Club {club_id} has a total seat count of 0 or less — it must be greater than 0",
    },
    club_reserve_exceeds_capacity: {
      vi: "CLB {club_id} có suất dự trữ lớn hơn tổng chỗ",
      en: "Club {club_id} has more reserved seats than total seats",
    },
    applicants_unknown_club: {
      vi: "Danh sách đăng ký thi tham chiếu tới CLB không tồn tại: {club_id}",
      en: "The tryout registration list references a club that does not exist: {club_id}",
    },
    applicants_unknown_student: {
      vi: "Danh sách đăng ký thi tham chiếu tới học sinh không tồn tại: {student_id}",
      en: "The tryout registration list references a student that does not exist: {student_id}",
    },
    ...
    club_over_capacity: {
      vi: "CLB {club_id} có {count} học sinh nhưng chỉ {capacity} chỗ — vượt quá tổng chỗ",
      en: "Club {club_id} has {count} student(s) holding seats but only {capacity} seat(s) total — over capacity",
    },
    club_over_reserve_capacity: {
      vi: "CLB {club_id} có {count} học sinh giữ suất dự trữ nhưng chỉ có {reserve_capacity} suất — vượt quá suất dự trữ",
      en: "Club {club_id} has {count} student(s) holding a reserved seat but only {reserve_capacity} reserved seat(s) exist — over the reserve limit",
    },
    blocking_pair: {
      vi: "Có thể chưa công bằng: học sinh {student_id} thích CLB {club_id} hơn CLB hiện tại {current_club}, và đủ điều kiện được nhận vào {club_id} nếu xét lại từ đầu (CLB đó hiện có {n_holders}/{capacity} chỗ đã nhận).",
      en: "Possible fairness gap: student {student_id} prefers club {club_id} over their current club {current_club}, and would qualify for a seat there if it were re-evaluated from scratch (that club currently holds {n_holders}/{capacity} seats).",
    },
    ...
    csv_student_missing_skipped: {
      vi: "{student_id}: chưa có trong hệ thống, bị bỏ qua vì chưa bật tuỳ chọn tự tạo học sinh mới.",
      en: "{student_id}: not yet in the system, skipped because auto-creating new students is turned off.",
    },
    score_not_applicant: {
    ...
    score_not_a_number: {
      vi: "{student_id}: điểm '{score}' không phải số — ô điểm đó KHÔNG được lưu.",
      en: "{student_id}: score '{score}' is not a number — that score was NOT saved.",
    },
    score_negative: {
    ...
    health_scoring_none: {
      vi: "CLB {club_id}: có {n_applicants} học sinh đăng ký thi nhưng CHƯA CHẤM ĐIỂM AI. Toàn bộ các em này sẽ rơi xuống nhóm xét sau (chỉ dựa vào số bốc thăm, không có điểm) — vòng thi coi như không có tác dụng.",
      en: "Club {club_id}: {n_applicants} student(s) registered for the tryout but NOBODY has been scored. All of them drop to the lottery-only group (no score to rank by) — the tryout will have no effect.",
    },
    ...
    health_orphan_student_group: {
      vi: "Nhãn dự trữ \"{reserve_group}\" đang gán cho {n} học sinh nhưng KHÔNG CLB nào dùng nhãn này (ví dụ: {sample}) — các em đó sẽ không được ưu tiên ở đâu cả, có thể do gõ sai chính tả. Sửa ở thẻ Quản lý: tìm mã em, đánh dấu, để trống ô nhãn rồi bấm \"Gán cho học sinh đã đánh dấu\".",
      en: "Reserve label \"{reserve_group}\" is assigned to {n} student(s) but NO club uses it (for example: {sample}) — those students get no priority anywhere, possibly a typo. Fix it in the Admin tab: search the student id, tick it, leave the label box empty, and click \"Assign to ticked students\".",
    },
    ...
    recovery_no_backups: {
      ...
      en: "No backups found — the allocation may never have run on this machine, or the backup folder was deleted.",
    },
  ...
  const UI_STRINGS = {
    vi: {
      ...
      nav_results: "Kết quả",
      nav_fallback: "Nhập tại chỗ",
      nav_admin: "Quản lý CLB & dự trữ",
      nav_scoring: "Nhập điểm",
      ...
      pipeline_desc: "Kiểm tra dữ liệu, sinh số bốc thăm, xếp học sinh vào CLB, và xuất kết quả.",
      ...
      steps_panel_title: "Các bước xử lý",
      label_seed: "Hạt giống bốc thăm",
      ...
      step_title_rbda: "Xếp học sinh vào CLB",
      step_title_write: "Lưu kết quả",
      step_title_export: "Xuất kết quả",
      ...
      fallback_title: "Nhập tại chỗ",
      fallback_desc: "Tìm hoặc tạo học sinh trước, sau đó có hai bước độc lập: chọn CLB để thi và xếp hạng nguyện vọng — không ảnh hưởng lẫn nhau.",
      fallback_step0_title: "Tìm hoặc tạo học sinh",
      ...
      fallback_tag_tested: "{n} CLB thi",
      fallback_tag_ranked: "{n} nguyện vọng",
      ...
      admin_title: "Quản lý CLB & dự trữ",
      admin_desc: "Tạo/sửa CLB và gán diện dự trữ cho học sinh. Tiêu chí dự trữ hoàn toàn do trường tự đặt — hệ thống chỉ so khớp nhãn dự trữ giữa CLB và học sinh, không cài cứng quy tắc nào.",
      ...
      admin_student_empty: "Chưa có học sinh nào trong hệ thống — thêm qua thẻ \"Nhập tại chỗ\" hoặc nạp từ Microsoft Forms.",
      btn_prev_page: "‹ Trước",
      btn_next_page: "Sau ›",
      ...
      danger_zone_safety: "Cả hai nút đều TỰ SAO LƯU app.db trước khi xoá, và đều KHÔNG xoá nhật ký các lần chạy — dấu vết kiểm toán được giữ nguyên.",
      ...
      /* --- Nhiều buổi sinh hoạt trong tuần --- */
      buoi_chua_chia: "Chưa chia buổi",
      th_buoi: "Buổi",
      placeholder_buoi: "Buổi sinh hoạt (vd: thu_3)",
      buoi_hint_form: "Hai CLB cùng một buổi là trùng giờ, học sinh chỉ vào được một. Bỏ trống nếu trường chỉ tổ chức một buổi.",
      admin_lich_tuan_title: "Lịch tuần",
      admin_lich_tuan_desc: "Mỗi cột là một buổi sinh hoạt. Thanh dưới mỗi CLB là tỉ lệ lấp đầy của lần chạy gần nhất.",
      admin_tai_buoi_title: "Tải theo buổi",
      admin_tai_buoi_desc: "Xem trước KHI CHƯA chạy phân bổ. Buổi nào chọi cao mà buổi khác còn trống thì dời bớt một CLB sang buổi vắng là sửa được, thay vì chạy xong mới đi giải thích vì sao nhiều em trượt.",
      th_so_clb: "Số CLB",
      th_tong_cho: "Tổng chỗ",
      th_em_muon_buoi: "Em muốn buổi này",
      th_ti_le_choi: "Tỉ lệ chọi",
      ti_le_choi_giai_thich: "Tỉ lệ chọi đếm theo SỐ HỌC SINH chứ không phải số lượt nguyện vọng, vì mỗi em chỉ lấy được một chỗ trong một buổi.",
      tai_buoi_qua_tai: "Quá tải",
      tai_buoi_vua_du: "Vừa đủ",
      tai_buoi_con_trong: "Còn trống",
      tai_buoi_empty: "Chưa có CLB nào để tính tải.",
      boc_tham_title: "Cách bốc thăm qua các buổi",
      boc_tham_desc: "Chỉ có ý nghĩa khi trường tổ chức nhiều buổi. Một buổi thì cả ba cách cho cùng một kết quả.",
      boc_tham_stb_tuan: "Bốc thăm một lần cho cả tuần",
      boc_tham_stb_tuan_hint: "Mỗi em một số dùng chung mọi buổi. Đơn giản và dễ giải thích nhất, nhưng em bốc phải số xấu sẽ đứng cuối ở MỌI buổi — may rủi cộng dồn.",
      boc_tham_stb_ngay: "Bốc thăm lại mỗi buổi",
      boc_tham_stb_ngay_hint: "Mỗi buổi xáo lại thứ tự từ chính bộ số đã khoá. May rủi san đều qua các buổi, nên ít em trắng tay cả tuần hơn.",
      boc_tham_stb_co_bu: "Bốc thăm có bù",
      boc_tham_stb_co_bu_hint: "Em chưa có CLB nào được xét trước ở buổi sau. Nghe công bằng, nhưng nó làm ưu tiên buổi sau phụ thuộc kết quả buổi trước — mở đường cho việc cố ý bỏ trống một buổi để giành ưu tiên buổi khác.",
      boc_tham_canh_bao_co_bu: "Cách này phá tính độc lập giữa các buổi. Hãy đọc bảng đối chiếu trước khi dùng nó cho một đợt phân bổ thật.",
      btn_so_sanh_boc_tham: "So sánh ba cách trên dữ liệu này",
      so_sanh_title: "Ba cách bốc thăm cho kết quả gì",
      so_sanh_desc: "Chạy thử cả ba cách ngay trên dữ liệu đang có. Đây chỉ là xem trước: không ghi kết quả, không thêm dòng nhật ký, không đụng tới số bốc thăm của ai.",
      so_sanh_dang_chay: "Đang thử cả ba cách…",
      th_cach_boc_tham: "Cách bốc thăm",
      th_trang_tay: "Trắng tay cả tuần",
      th_tb_clb: "CLB trung bình mỗi em",
      th_do_lech: "Độ lệch",
      th_hang_tb: "Hạng nguyện vọng TB",
      th_pha_vo: "Cặp phá vỡ",
      th_o_khac: "Ô xếp khác",
      so_sanh_moc_label: "mốc",
      so_sanh_mot_buoi: "Trường đang chỉ có một buổi sinh hoạt, nên cả ba cách cho đúng cùng một kết quả. Bảng này chỉ có ý nghĩa khi có từ hai buổi trở lên.",
      so_sanh_cach_doc: "Cột “Ô xếp khác” đếm số ô (học sinh, buổi) xếp khác so với cách đầu bảng. Các cột tổng có thể gần bằng nhau trong khi kết quả vẫn khác hẳn — đọc cả hai trước khi kết luận ba cách như nhau.",
      results_xem_tkb: "Thời khoá biểu",
      results_xem_danh_sach: "Danh sách",
      tkb_title: "Thời khoá biểu tuần",
      tkb_desc: "Mỗi em một dòng, mỗi buổi một ô. Ô trống nghĩa là buổi đó em không có CLB nào.",
      tkb_so_clb: "Số CLB",
      do_phu_title: "Độ phủ theo học sinh",
      do_phu_desc: "Con số đáng nhìn nhất là số em không có CLB nào trong cả tuần — tỉ lệ lấp đầy từng CLB không cho thấy điều đó.",
      do_phu_tb: "CLB trung bình mỗi em",
      do_phu_trang_tay: "Em không có CLB nào cả tuần",
      do_phu_x_clb: "{n} CLB",
      do_phu_0_clb: "Không CLB nào",
      do_phu_1_clb: "1 CLB",
      do_phu_danh_sach: "Danh sách em trắng tay",
      fallback_buoi_hint: "Mỗi buổi em chỉ nhận được nhiều nhất một CLB. Xếp hạng CLB ở nhiều buổi khác nhau thì tuần của em mới kín.",
      fallback_phu_buoi: "Đã xếp hạng ở {n}/{tong} buổi",
    },
    ...
    en: {
      ...
      nav_results: "Results",
      nav_fallback: "Kiosk entry",
      nav_admin: "Clubs & reserves",
      nav_scoring: "Score entry",
      ...
      pipeline_desc: "Validate data, draw lottery numbers, match students to clubs, and export results.",
      ...
      steps_panel_title: "Processing steps",
      label_seed: "Lottery seed",
      ...
      step_title_rbda: "Match students to clubs",
      step_title_write: "Save results",
      step_title_export: "Export results",
      ...
      results_empty_state: "No results yet — run it from the \"Pipeline\" tab first.",
      not_matched_label: "Not matched",
      unmatched_badge: "{n} not matched",
      tier_general: "General",
      tier_reserve: "Reserve",

      fallback_title: "Kiosk entry",
      fallback_desc: "Find or create the student first, then complete two independent steps: clubs to test for, and ranked preferences — neither affects the other.",
      fallback_step0_title: "Find or create a student",
      ...
      fallback_tag_tested: "{n} tested clubs",
      fallback_tag_ranked: "{n} preferences",
      ...
      admin_title: "Clubs & reserves",
      admin_desc: "Create/edit clubs and assign reserved status to students. Reserve criteria are entirely up to the school — the system only matches the reserve label between clubs and students, nothing is hardcoded.",
      ...
      admin_student_empty: "No students in the system yet — add them via the \"Kiosk entry\" tab or import from Microsoft Forms.",
      btn_prev_page: "‹ Prev",
      btn_next_page: "Next ›",
      ...
      danger_zone_safety: "Both buttons BACK UP app.db before deleting anything, and neither touches the run log — the audit trail is kept.",
      ...
      btn_redraw_stb: "Redraw lottery numbers…",
      ...
      buoi_chua_chia: "No session set",
      th_buoi: "Session",
      placeholder_buoi: "Session (e.g. thu_3)",
      buoi_hint_form: "Two clubs in the same session clash, so a student can only join one of them. Leave it blank if your school runs a single session.",
      admin_lich_tuan_title: "The week at a glance",
      admin_lich_tuan_desc: "Each column is one session. The bar under each club shows how full it was on the latest run.",
      admin_tai_buoi_title: "Demand by session",
      admin_tai_buoi_desc: "Read this before you run the allocation. If one session is crowded while another has room, moving a club to the quieter session fixes the problem — far better than explaining afterwards why so many students missed out.",
      th_so_clb: "Clubs",
      th_tong_cho: "Seats",
      th_em_muon_buoi: "Students wanting it",
      th_ti_le_choi: "Applicants per seat",
      ti_le_choi_giai_thich: "Applicants per seat counts STUDENTS, not ranked entries — each student can take only one seat per session.",
      tai_buoi_qua_tai: "Oversubscribed",
      tai_buoi_vua_du: "About right",
      tai_buoi_con_trong: "Room to spare",
      tai_buoi_empty: "No clubs yet, so there is nothing to measure.",
      boc_tham_title: "Lottery design across sessions",
      boc_tham_desc: "This only matters when your school runs several sessions. With a single session all three designs give the same result.",
      boc_tham_stb_tuan: "One draw for the whole week",
      boc_tham_stb_tuan_hint: "Each student gets one number, used in every session. It is the simplest design to explain, but a student with a poor number sits at the back in every single session — bad luck compounds.",
      boc_tham_stb_ngay: "A fresh draw each session",
      boc_tham_stb_ngay_hint: "Each session reshuffles the order, derived from the same locked draw. Luck evens out across the week, so fewer students end up with nothing at all.",
      boc_tham_stb_co_bu: "Compensating draw",
      boc_tham_stb_co_bu_hint: "Students with nothing yet move to the front in later sessions. It sounds fair, but it makes a later session's priority depend on an earlier session's outcome — which rewards deliberately skipping one session to get ahead in another.",
      boc_tham_canh_bao_co_bu: "This design breaks the independence between sessions. Read the comparison below before using it for a real allocation.",
      btn_so_sanh_boc_tham: "Compare all three on this data",
      so_sanh_title: "What each lottery design would produce",
      so_sanh_desc: "Runs all three designs against the data you have loaded. This is a preview only: nothing is saved, no run is logged, and nobody's lottery number is touched.",
      so_sanh_dang_chay: "Trying all three designs…",
      th_cach_boc_tham: "Design",
      th_trang_tay: "Nothing all week",
      th_tb_clb: "Clubs per student",
      th_do_lech: "Spread",
      th_hang_tb: "Mean choice rank",
      th_pha_vo: "Blocking pairs",
      th_o_khac: "Placements changed",
      so_sanh_moc_label: "baseline",
      so_sanh_mot_buoi: "Your school currently has a single session, so all three designs give exactly the same result. This table only becomes meaningful with two or more sessions.",
      so_sanh_cach_doc: "“Placements changed” counts the (student, session) cells that differ from the first row. The summary columns can look almost identical while the actual placements differ a lot — read both before concluding the designs are equivalent.",
      results_xem_tkb: "Timetable",
      results_xem_danh_sach: "List",
      tkb_title: "The week each student gets",
      tkb_desc: "One row per student, one cell per session. An empty cell means no club for that student in that session.",
      tkb_so_clb: "Clubs",
      do_phu_title: "How many clubs each student got",
      do_phu_desc: "The number worth watching is how many students end the week with nothing. Per-club fill rates never reveal it.",
      do_phu_tb: "Clubs per student",
      do_phu_trang_tay: "Students with no club all week",
      do_phu_x_clb: "{n} clubs",
      do_phu_0_clb: "No club",
      do_phu_1_clb: "1 club",
      do_phu_danh_sach: "Students with nothing",
      fallback_buoi_hint: "A student can get at most one club per session. Ranking clubs across different sessions is what fills up their week.",
      fallback_phu_buoi: "Ranked in {n} of {tong} sessions",
    },
  };
